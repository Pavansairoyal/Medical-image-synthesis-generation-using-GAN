...
def yolo_detect_disease(image_path):
    """Detect skin condition using YOLOv8 classification model"""
    from PIL import Image
    ...
    names_dict = results[0].names  # e.g., {0: "Melanocytic nevi", 1: "Melanoma", ...}
    ...
    class_name = names_dict.get(top_index, "Unknown")
    confidence = round(probs[top_index], 2)

    return {
        "detections": [{
            "class_name": class_name,
            "confidence": confidence,
            "description": disease_info.get(class_name)["description"],
            "treatment": disease_info.get(class_name)["treatment"]
        }]
    }
